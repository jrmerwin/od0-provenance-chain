...
# step by stepC26 Stagegate Checklist
Step 1: Patch and Certify (Local Verification)

Run python3 make_v21i4_c26.py in the bundle directory. Verify the printed SHA-256 hash matches Fable's output (69e808c7...f4abd) on both your Mac and Windows machines.

Run python3 c26_driver.py certify c26_config.json. This must output PASS x2 and pass the identity keystone gate on each machine.

Step 2: Install the Archived C6/C13 Metric Adapter

Open c26_formation_adapter.py.

Replace the temporary native-pulse fallback code with your exact archived C6/C13 metric adapter.

Certify the adapter by running it against one of your archived C13/C15 formation rows to ensure it exactly reproduces the prior behavior. (The driver will refuse to run frozen until this is done).

Step 3: Ratify the Implementation Freeze-Points
Review and confirm the 7 physical definitions in the code before freezing. The two most critical are:

The d/ℓ* definition: Ensure Fable's rendering (weighted face-distance divided by the deepest candidate member scale) exactly matches the definition you used and archived in Round C24.

Snapshot-Lattice Support Math: Because memory constraints require snapshots every 10 epochs (snap_every=10), the preregistered "≥90 frustrated-face epochs" is translated to support_min_snaps=9. Confirm this scaling is acceptable.

Update c26_config.json with the ratified values.

Step 4: Execute the Pre-Registration Freeze

Run python3 c26_freeze.py C26_prereg_dual_candidate_structural_preflight.md.

This will generate a deterministic, cryptographically hashed seed list (c26_seeds.json) and a FREEZE_MANIFEST.json.

Commit both of these files to your Git repository alongside the preregistration document. This locks the experiment.

Step 5: Run the Frozen Preflight

Execute python3 c26_driver.py all c26_config.json [budget_seconds].

Run this in FROZEN mode on your Mac/Windows hardware. You can use the budget argument to chunk the long runs without breaking the cache.

Once those five steps are complete, the C26 preflight will be churning on your hardware, and you can immediately pivot to Track 2 (the depth-contrast boundary candidate) with Fable.
